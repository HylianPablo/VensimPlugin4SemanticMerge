package es.uva.inf.vensimparserforsemanticmerge.comment;
...
import es.uva.inf.vensimparserforsemanticmerge.formatters.Comment;
...
public class CommentTest {
    ...
    public void lecturaCorrectaEcuacionDescriptiva() {
        ...
        try {
            ...
            positions.addAll(Arrays.asList(
                    new Integer[] { 4, 10, 16, 22, 28, 34, 40, 46, 52, 58, 63, 68, 73, 78, 83, 88, 93, 98, 103, 108 }));
            for (int i = 0; i < 118; i++) {
                String line = reader.readLine();
                if (positions.contains(i)) {
                    assertTrue(line.indexOf("View 1") != -1);
    ...
    public void lecturaCorrectaConComentarios() {
        ...
        try {
            ...
            Set<Integer> positions = new HashSet<Integer>();
            positions.addAll(Arrays.asList(new Integer[] { 4, 10, 16, 22, 28, 34, 40, 46, 52, 58, 63, 68, 74, 80, 86,
                    91, 96, 101, 106, 111 }));
            for (int i = 0; i < 118; i++) {
                String line = reader.readLine();
                if (positions.contains(i)) {
