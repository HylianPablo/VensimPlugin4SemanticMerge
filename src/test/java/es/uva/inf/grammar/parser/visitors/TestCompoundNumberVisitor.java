...
import static es.uva.inf.grammar.testutilities.GeneralTestUtilities.getParseTreeFromString;
...
public class TestCompoundNumberVisitor {
    ...
    private GrammarParser.CallContext getAloneCallExprFromProgram(String program) {
        GrammarParser.FileContext tree = getParseTreeFromString(program);

        return ((GrammarParser.CallExprContext) tree.model().symbolWithDoc(0).symbolWithDocDefinition().equation().expr().children.get(0)).call();
    }
    ...
    public void testValidCompoundNestedCalls(){
        ...
        GrammarParser.CallContext call = getAloneCallExprFromProgram(program);
